...
def resblock_up(inputs,
    ...
    with tf.variable_scope(scope_name, reuse=reuse):
        ...
        conv1 = tf.layers.conv3d_transpose(inputs,
                                           filters_in,
                                           (3, 3, 3),
                                           strides,
                                           padding='same',
        ...
        conv2 = tf.layers.conv3d_transpose(conv1,
                                           filters_out,
                                           (3, 3, 3),
                                           (1, 1, 1),
                                           padding='same',
        ...
        conv3 = tf.layers.conv3d_transpose(inputs,
                                           filters_out,
                                           (3, 3, 3),
                                           strides,
                                           padding='same',
...
class VariationalAutoencoder():
    ...
    def encoder(self, x, reuse=False, is_train=True):
        ...
        with tf.variable_scope(self.model_name+"_encoder", reuse=reuse):
            ...
            # =========================================================
            res1 = resblock_down(conv1, gf_dim, gf_dim, "res1", reuse, is_train, strides=(2, 2, 2))

            res2 = resblock_down(res1, gf_dim, gf_dim * 2, "res2", reuse, is_train, strides=(2, 2, 2))

            res3 = resblock_down(res2, gf_dim * 2, gf_dim * 4, "res3", reuse, is_train, strides=(2, 2, 2))

            res4 = resblock_down(res3, gf_dim * 4, gf_dim * 8, "res4", reuse, is_train, strides=(2, 2, 1))

            # =========================================================
            # Convolutions to compute latent mu and latent standard deviation
            # =========================================================
            conv_latent = tf.layers.conv3d(res4,
                                           gf_dim*32,
                                           (1,1,1),
                                           padding='same',
                                           kernel_initializer=w_init,
                                           ...
                                           name="latent_mu",
                                           reuse=reuse)
            conv_latent_std = tf.layers.conv3d(res4,
                                               gf_dim * 32,
                                               (1, 1, 1),
                                               padding='same',
                                               kernel_initializer=w_init,
            ...
            conv4 = tf.layers.conv3d(conv3,
                                     ...
                                     name="stddev_conv4",
                                     reuse=reuse)
            conv4 = tf.layers.batch_normalization(conv3,
                                                  center=True,
                                                  scale=True,
                                                  trainable=True,
                                                  training=is_train,
            ...
            # ====================================
            logging.info('===================== ENCODER NETWORK ====================== ')
            logging.info('Shape of input: ' + str(x.shape))
            logging.info('Shape after 1st convolution block: ' + str(conv1.shape))
            logging.info('Shape after 1st res block: ' + str(res1.shape))
            logging.info('Shape after 2nd res block: ' + str(res2.shape))
            logging.info('Shape after 3rd res block: ' + str(res3.shape))
            logging.info('Shape after 4th res block: ' + str(res4.shape))
            logging.info('-------------------------------------------------')
            logging.info('Shape of latent_Mu: ' + str(conv_latent.shape))
            logging.info('Shape of latent_stddev: ' + str(conv_latent_std.shape))
            logging.info('-------------------------------------------------')
            logging.info('=========================================================== ')
    ...
    def decoder(self, z, reuse=False, is_train=True):
        ...
        with tf.variable_scope("predictor" , reuse=reuse):
            ...
            logging.info(' Input to decoder has the following shape:' + str(z.shape))

            # =========================================================
            # Res-Blocks (for effective deep architecture)
            # =========================================================
            resp1 = resblock_up(z, gf_dim * 32, gf_dim * 16, "decode_resp1", reuse, is_train, strides=(2, 2, 1))

            res0 = resblock_up(resp1, gf_dim * 16, gf_dim * 8, "decode_res0", reuse, is_train, strides=(2, 2, 2))

            res1 = resblock_up(res0, gf_dim * 8, gf_dim * 4, "decode_res1", reuse, is_train, strides=(2, 2, 2))

            res2 = resblock_up(res1, gf_dim * 4, gf_dim * 2, "decode_res2", reuse, is_train, strides=(2, 2, 2))
            ...
            logging.info('===================== DECODER NETWORK ====================== ')
            logging.info('Shape of input: ' + str(z.shape))
            logging.info('Shape after 1st convolution block: ' + str(resp1.shape))
            logging.info('Shape after 1st res block: ' + str(res0.shape))
            logging.info('Shape after 2nd res block: ' + str(res1.shape))
            logging.info('Shape after 3rd res block: ' + str(res2.shape))
            #logging.info('Shape after 4th res block: ' + str(res3.shape))
            #logging.info('Shape after 5th res block: ' + str(res4.shape))
            logging.info('-------------------------------------------------')
            logging.info('Shape after 1st convolution ' + str(conv1.shape))
            logging.info('Shape of output of decoder' + str(conv2.shape))
            logging.info('-------------------------------------------------')
            logging.info('=========================================================== ')
