...
export DEPLOY_IMAGE=${DEPLOY_IMAGE:-"${CONTAINER_REGISTRY}/hpc-predict/anomaly_detection/deploy:${COMMIT_SHA}"}
export DEPLOY_IMAGE_UNTAGGED=${DEPLOY_IMAGE_UNTAGGED:-"${CONTAINER_REGISTRY}/hpc-predict/anomaly_detection/deploy"}
...
echo "Run \"export HPC_PREDICT_ANOMALY_DETECTION_IMAGE=${DEPLOY_IMAGE_UNTAGGED}\" or \"export HPC_PREDICT_ANOMALY_DETECTION_IMAGE=${DEPLOY_IMAGE}\" to automatically use the built image with docker/run scripts."
cd -
